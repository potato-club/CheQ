...
class S_Keychain {
    ...
    */
    ...
    private func createDeviceID() -> Bool {
    ...
    public func getDeviceID() -> String {
        ...
        if status == errSecItemNotFound || status == -25300 {
            let create = createDeviceID()
            if !create {
                return ""
            }
        ...
        if status == errSecSuccess {
            guard let existingItem = dataTypeRef as? [String: Any] else {
                print("")
    ...
    private func setDeviceID() -> String {
